...
IMAGE_DIR = 'images'
OUTPUT_CSV = 'subset_100_with_images.csv'
TARGET_COUNT = 100
...
def main():
    ...
    # 상위 100개의 고유한 parent_asin 추출
    target_asins = df['parent_asin'].dropna().unique()[:TARGET_COUNT]
        ...
        for line in f:
            try:
                ...
                if asin in target_asin_set:
                    ...
                    # 모든 타겟 ASIN을 다 찾았으면 조기 종료
                    if len(asin_to_url) == len(target_asin_set):
                        break
    ...
    print(f"다운로드 성공: {success_count} / {len(download_tasks)}")

    print("4. 최종 데이터셋 생성")
    # 타겟 ASIN에 해당하는 행만 필터링
    filtered_df = df[df['parent_asin'].isin(target_asin_set)].copy()
    
    # 다운로드 성공한 로컬 이미지 경로 매핑
    filtered_df['image_path'] = filtered_df['parent_asin'].map(asin_to_local_path)
