...
def get_metrics(weights_path, model_name, val_loader, df):
...
def main():
    ...
    os.makedirs("model_evaluate", exist_ok=True)
    out_path = os.path.join("model_evaluate", "v9_evaluation_results.txt")
    
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("================ V9 COMPARISON ================\n")
        if v1_metrics:
            f.write(f"Model V9 v1 (Existing):\n")
            f.write(f"  - MAE: {v1_metrics['MAE']:.4f}\n")
            f.write(f"  - MSE: {v1_metrics['MSE']:.4f}\n")
            f.write(f"  - R2 Score: {v1_metrics['R2']:.4f}\n\n")
        if v2_metrics:
            f.write(f"Model V9 v2 (With Pretrained weights):\n")
            f.write(f"  - MAE: {v2_metrics['MAE']:.4f}\n")
            f.write(f"  - MSE: {v2_metrics['MSE']:.4f}\n")
            f.write(f"  - R2 Score: {v2_metrics['R2']:.4f}\n")
        f.write("===============================================\n")
