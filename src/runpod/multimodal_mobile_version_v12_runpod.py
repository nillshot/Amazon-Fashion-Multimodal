"""
Multimodal Fashion Model V12 - RunPod Optimized Version
=======================================================
V12 일반 버전의 메인 모델 구조는 유지하고, RunPod 실행 안정화 기능만 추가한 버전입니다.

V12 유지 사항:
  - seller_text / review_text 분리 입력
  - Concatenate-Attend-Split 기반 Cross-Attention
  - Review Modality Dropout
  - Gate Entropy Loss
...
사용 예:
  python multimodal_mobile_version_v12_runpod.py \
      --data-csv /workspace/fashion_train_subset_3_with_meta_text_v10.csv \
      --output-dir /workspace/outputs_v12 \
      --epochs 10 \
      --phase1-epochs 2 \
      --batch-size 2 \
      --accum-steps 4 \
  ...
  python multimodal_mobile_version_v12_runpod.py --smoke-test
...
# V12 유지: 일반 버전과 동일한 Review Dropout 확률
REVIEW_DROPOUT_P = 0.30
...
def setup_logger(output_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("v12_train")
    logger.setLevel(logging.INFO)
    logger.handlers.clear()

    fmt = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s", datefmt="%Y-%m-%d %H:%M:%S")
    fh = logging.FileHandler(output_dir / "train_v12.log", encoding="utf-8")
    fh.setFormatter(fmt)
...
def parse_args():
    ...
    p.add_argument("--output-dir", type=str, default="/workspace/outputs_v12")  # V10 변경
...
# 2. 데이터셋 클래스
# ==========================================
class AmazonFashionV12Dataset(Dataset):
...
def weighted_mse_loss(pred, target):
    """V12 RunPod: 일반 버전과 동일한 weighted MSE를 벡터화하여 속도 개선"""
...
def train_epoch(model, loader, optimizer, scheduler, scaler, device, acc_steps, epoch, logger, deadline_ts, use_amp):
    ...
    for i, batch in enumerate(pbar):
        ...
        with autocast(enabled=use_amp):
            out_fused, out_seller, out_review, out_image, gate_weights, seller_pooled, i_seq, avg_t_attn_w, t_attended_pooled = model(
            ...
            loss_task = loss_fused + 0.3 * loss_seller + 0.3 * loss_review + 0.3 * loss_image  # V12 유지: 보조 loss 균형화
            ...
            loss = (
                ...
                + 0.1 * loss_ccr
                + 0.1 * loss_ccs
                + GATE_ENTROPY_LAMBDA * loss_gate_ent  # V12 유지: 약한 Gate Entropy Loss
            ) / acc_steps
...
def main():
    ...
    logger.info(f"V12 RunPod Args: {vars(args)}")
    ...
    if "seller_text" not in df.columns or "review_text" not in df.columns:
        raise ValueError("V12 CSV에는 seller_text, review_text 컬럼이 필요합니다.")
    ...
    train_ds = AmazonFashionV12Dataset(train_df, tokenizer, train_transform)
    val_ds = AmazonFashionV12Dataset(val_df, tokenizer, val_transform)
    ...
    logger.info(f"Model V12 params: {n_params:.1f}M")
    logger.info(f"[V12] Review Dropout P = {REVIEW_DROPOUT_P}")
    logger.info(f"[V12] Gate Entropy Lambda = {GATE_ENTROPY_LAMBDA}")
    logger.info("[V12] 메인 모델 구조는 일반 V12와 동일하게 유지")

    scaler = GradScaler(enabled=use_amp)
    last_ckpt_path = output_dir / "last_ckpt_v12_0.01.pth"
    best_ckpt_path = output_dir / "best_mobile_version_v12_model_0.01.pth"
    ...
    if args.resume and last_ckpt_path.exists():
        ...
        logger.warning("--resume이 지정됐지만 last_ckpt_v12.pth가 없어 처음부터 시작합니다.")
    ...
    try:
        ...
        for epoch in range(phase2_start, args.epochs + 1):
            ...
            if not interrupted:
                ...
                if val_mae < best_mae:
                    ...
                    logger.info(f"🌟 최적 V12 가중치 저장: MAE={best_mae:.4f} -> {best_ckpt_path}")
        ...
        logger.warning(f"⏰ {e} — last_ckpt_v12.pth 저장 완료. 재실행 시 --resume을 사용하세요.")
    except KeyboardInterrupt:
        logger.warning("사용자 중단 감지 — 현재까지의 체크포인트는 last_ckpt_v12.pth에 저장됩니다.")
